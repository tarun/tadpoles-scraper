...
if (config.cookie_header) {
	cookieJar.setCookie(
		tough.Cookie.parse(config.cookie_header)
		, "https://www.tadpoles.com", {}, console.log)
}
	...
	if (!fs.existsSync(dirbase + filebase + '.jpg') && !fs.existsSync(dirbase + filebase + '.png') && !fs.existsSync(dirbase + filebase + '.mp4') && !fs.existsSync(dirbase + filebase + '.pdf')) {
		...
		try {
			await client.get(imgurl, {
				...
				if (extension) {
					filename = dirbase + filebase + '.' + extension
					fs.writeFileSync(filename, response.data)
				} else {
					console.log('      -- content-type ' + contentType + ' excluded - skipping')
				}
			})
			return filename
		} catch (error) {
		...
		for (const event of list.data.events) {
			for (const imgkey of event.attachments) {
				...
				try {
					...
					// ignore error gracefully proceed to next item, if there is an error downloading or processing
					console.error(e)
				}
			}
		}
